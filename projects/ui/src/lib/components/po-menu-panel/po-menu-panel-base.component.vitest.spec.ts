...
describe('PoMenuPanelBaseComponent: ', () => {
  ...
  it('should be created', () => {
    expect(component instanceof PoMenuPanelBaseComponent).toBe(true);
  });
  ...
  describe('Properties: ', () => {
    ...
    it('menus: should set `menus` with a array empty when invalid values', () => {
      ...
      const spySetMenu = vi.spyOn(component as any, 'setMenuExtraProperties');
      const spyValidate = vi.spyOn(component as any, 'validateMenus');

      expectPropertiesValues(component, 'menus', invalidValues, []);

      expect(spySetMenu).toHaveBeenCalled();
