...
describe('PoHelperComponent', () => {
  ...
  describe('emitClick', () => {
    ...
    it('should return early when helper is falsy (undefined) and not call handleEmitEvent', () => {
      ...
      const handleSpy = vi.spyOn<any, any>(component, 'handleEmitEvent');
  ...
  describe('onKeyDown', () => {
    let popover: { open: ReturnType<typeof vi.fn>; close: ReturnType<typeof vi.fn>; isHidden: boolean };
  ...
  it('should call detectChanges on ngOnChanges when size changes', () => {
    const spy = vi.spyOn(component['cdr'], 'detectChanges');
  ...
  describe('handleClose', () => {
    ...
    it('should not remove focusin listener when boundFocusIn is undefined', () => {
      const removeSpy = vi.spyOn(window, 'removeEventListener');
      removeSpy.mockClear(); // Ensure no calls from previous test cleanup
