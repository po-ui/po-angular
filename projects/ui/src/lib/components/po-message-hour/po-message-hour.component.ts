...
export const poMessageHourDefault: Record<'en' | 'es' | 'pt' | 'ru', PoMessageHourLiterals> = {
...
export class PoMessageHourComponent extends PoMessageHourBaseComponent {
  public literals!: PoMessageHourLiterals;

  message!: string;

  ngOnInit() {
    const browserLanguage = navigator.language.split('-')[0] as keyof typeof poMessageHourDefault;
    this.literals = poMessageHourDefault[browserLanguage] ?? poMessageHourDefault.en;
