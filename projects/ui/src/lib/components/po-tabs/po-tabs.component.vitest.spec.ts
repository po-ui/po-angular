...
import { CUSTOM_ELEMENTS_SCHEMA, NO_ERRORS_SCHEMA, EventEmitter, QueryList } from '@angular/core';
...
describe('PoTabsComponent:', () => {
  ...
  describe('Methods:', () => {
    ...
    it('ngOnInit: should call `onTabActiveByDropdown` if tab is active in tabsDropdown', () => {
      const tabMock = { id: 'tab-id' } as any;

      vi.spyOn(component, 'onTabActiveByDropdown').mockImplementation(() => {});
    ...
    it('ngOnInit: should unsubscribe', () => {
      ...
      vi.spyOn(component['subscriptionTabsService'], 'unsubscribe');
      vi.spyOn(component['subscriptionTabActive'], 'unsubscribe');
    ...
    it('onTabChangeState: shouldn`t call `activeDistinctTab`, `detectChanges` if `tab.active` is false', () => {
      ...
      vi.spyOn(component as any, 'activeDistinctTab').mockImplementation(() => {});
    ...
    it('onTabActive: should set `previousActiveTab` and call `deactivateTab`', () => {
      ...
      vi.spyOn(component as any, 'deactivateTab').mockImplementation(() => {});
    ...
    it('activeFirstTab: should `active` first tab if it isn`t disabled or hide', () => {
      ...
      vi.spyOn(component, 'tabs', 'get').mockReturnValue([defaultTab]);
    ...
    it('activeFirstTab: shouldn`t `active` first tab if them are disabled or hide', () => {
      component.quantityTabsButton = 1;
      vi.spyOn(component, 'tabs', 'get').mockReturnValue([disabledTab, hiddenTab]);
      component['activeFirstTab']();
      const tab = component.tabs[0];
    ...
    it(`closeListbox: should call 'closeDropdown' if 'isDropdownOpen' is 'true'.`, () => {
      ...
      const spyOnCloseDropdown = vi.spyOn(fakeThis.tabDropdown, 'closeDropdown');
    ...
    it('activeDistinctTab: shouldn`t call `activeFirstTab` when `previousActiveTab` is true', () => {
      ...
      vi.spyOn(component as any, 'activeFirstTab').mockImplementation(() => {});
    ...
    it('handleArrowRight: should call setTabIndex in next tab if index is 1', () => {
      ...
      vi.spyOn(component as any, 'setTabIndex').mockImplementation(() => {});
      vi.spyOn(tabElements[indexArrow + 1], 'focus').mockImplementation(() => {});
    ...
    it('handleArrowRight: should call setTabIndex in the fisrt next tab if index is the last', () => {
      ...
      vi.spyOn(component as any, 'setTabIndex').mockImplementation(() => {});
      vi.spyOn(tabElements[0], 'focus').mockImplementation(() => {});
    ...
    it('handleArrowLeft: should call setTabIndex in the last tab if index is 0', () => {
      ...
      vi.spyOn(component as any, 'setTabIndex').mockImplementation(() => {});
      vi.spyOn(tabElements[tabElements.length - 1], 'focus').mockImplementation(() => {});
    ...
    it('should handleKeyDown correctly for Home key', () => {
      ...
      vi.spyOn(component as any, 'handleHomeKey').mockImplementation(() => {});
    ...
    it('should handleKeyDown correctly for End key', () => {
      ...
      vi.spyOn(event, 'preventDefault');
      vi.spyOn(event, 'stopPropagation');
      vi.spyOn(component as any, 'handleEndKey').mockImplementation(() => {});
    ...
    it('should handleKeyDown correctly for ArrowLeft key', () => {
      ...
      vi.spyOn(component as any, 'handleArrowLeft').mockImplementation(() => {});
    ...
    it('should handleKeyDown correctly for ArrowRight key', () => {
      ...
      vi.spyOn(component as any, 'handleArrowRight').mockImplementation(() => {});
    ...
    it('should add keydown event listeners', () => {
      ...
      vi.spyOn(component as any, 'setTabIndex').mockImplementation(() => {});
      vi.spyOn(component as any, 'handleKeyDown').mockImplementation(() => {});
    ...
    it('should call initializeTabAccessibilityElements if there is tabable focus', () => {
      ...
      vi.spyOn(component as any, 'initializeTabAccessibilityElements').mockImplementation(() => {});
    ...
    it('onTabActiveByDropdown: should correctly call methods and update styles when called', () => {
      const tabMock = { id: 'tab-id', click: { emit: vi.fn() }, activatedTab: { emit: vi.fn() } } as any;
      component.defaultLastTabWidth = 100;

      const nativeElementMock = { style: { width: '' }, getBoundingClientRect: () => ({ width: 100 }) };
      const queryListMock = { last: { nativeElement: nativeElementMock } } as any;
      ...
      const mockQueryList = {
        reset: vi.fn(),
        toArray: () => arr,
        find: (fn: any) => arr.find(fn),
        _results: arr
      } as any;
      component.tabsChildren = mockQueryList;
    ...
    it('reorderTabs: should reorder tab to position of lastTab and call reset and detectChanges', () => {
      ...
      vi.spyOn(component.tabsChildren, 'reset');
      vi.spyOn(component.changeDetector, 'detectChanges').mockImplementation(() => {});
    ...
    it('reorderTabs: should reorder tabs if tabIndex is not -1', () => {
      const tabMock = { id: 'tab4', widthButton: '130', click: { emit: vi.fn() } } as any;
      const tabsArrayMock = [
        { id: 'tab1', widthButton: '100' } as any,
        { id: 'tab2', widthButton: '110' } as any,
        { id: 'tab3', widthButton: '120' } as any,
        tabMock
      ];

      const tabsChildrenMock = {
        toArray: () => [...tabsArrayMock],
        find: (fn: any) => tabsArrayMock.find(fn),
        reset: vi.fn().mockImplementation((arr: any[]) => {
          tabsArrayMock.length = 0;
          tabsArrayMock.push(...arr);
          tabsChildrenMock.toArray = () => [...tabsArrayMock];
          tabsChildrenMock.find = (fn: any) => tabsArrayMock.find(fn);
        }),
        _results: tabsArrayMock
      } as any;
      Object.defineProperty(component, 'tabsChildren', { value: tabsChildrenMock, writable: true });

      component.quantityTabsButton = 3;

      const nativeElementMock = { style: { width: '' }, getBoundingClientRect: () => ({ width: 100 }) };
      component.tabButton = { last: { nativeElement: nativeElementMock } } as any;

      vi.spyOn(component as any, 'changeTabPositionByDropdown').mockImplementation((tab: any) => {
        const arr = tabsChildrenMock.toArray();
        const tabIndex = arr.findIndex((t: any) => t.id === tab.id);
        if (tabIndex !== -1) {
          arr.splice(tabIndex, 1);
          arr.splice(component.quantityTabsButton - 1, 0, tab);
          tabsChildrenMock.reset(arr);
        }
      });
      ...
      const reorderedTabs = tabsChildrenMock.toArray();

      expect(tabMock.click.emit).toHaveBeenCalledWith(tabMock);
      expect(reorderedTabs.map((tab: any) => tab.id)).toEqual(['tab1', 'tab2', 'tab4', 'tab3']);
    ...
    it('changeTabPositionByDropdown: should change tab position by dropdown when byContextsTabs is true', () => {
      ...
      vi.spyOn(component as any, 'onTabActive').mockImplementation(() => {});
    ...
    it('updateTabsState: should update tabs state correctly', () => {
      ...
      const tabsArrayMock = [
        { id: 'tab1', widthButton: 0 } as any,
        { id: 'tab2', widthButton: 100 } as any,
        { id: 'tab3', widthButton: 0 } as any
      ];
    ...
    it('selectedTab: should correctly handle a tab in the dropdown', () => {
      ...
      vi.spyOn(component, 'onTabActiveByDropdown').mockImplementation(() => {});
      const changeDetectorSpy = vi.spyOn(component['changeDetector'], 'detectChanges').mockImplementation(() => {});
    ...
    describe('activateTab', () => {
      it('should call selectedTab when the tab id is found in the array', () => {
        ...
        const spySelectedTab = vi.spyOn(component, 'selectedTab').mockImplementation(() => {});
