...
describe('PoHeaderBaseComponent', () => {
  ...
  let languageServiceSpy: { getShortLanguage: ReturnType<typeof vi.fn> };

  beforeEach(() => {
    languageServiceSpy = {
      getShortLanguage: vi.fn().mockReturnValue('pt')
    };
  ...
  describe('p-size', () => {
    ...
    it('onThemeChange: should recalculate size when theme changes', async () => {
      const spy1 = vi.spyOn(component['themeChangeSignal'], 'update');
      document.documentElement.setAttribute('data-a11y', PoThemeA11yEnum.AA);
    ...
    it('onThemeChange: should return an error when calling updateMenu', () => {
      ...
      vi.spyOn(component, 'updateMenu').mockImplementation(() => { throw new Error('DOM not ready'); });
