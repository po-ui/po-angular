  ...
  describe('Properties: ', () => {
    describe('p-icon: ', () => {
      it('should call `addClasses` if value is a string.', () => {
        const addClassesSpy = vi.spyOn(component as any, 'addClasses');

        const iconString = 'po-icon-user';
        component.icon = iconString;

        expect(addClassesSpy).toHaveBeenCalledWith(iconString);
      });
      ...
      it('should update `icon` value if value is a TemplateRef.', () => {
        ...
        expect(addClassesSpy).not.toHaveBeenCalled();
