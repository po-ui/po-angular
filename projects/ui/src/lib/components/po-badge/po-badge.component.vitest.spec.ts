...
describe('PoBadgeComponent:', () => {
  ...
  describe('Methods:', () => {
    it('setStatus: should call setStatus when "status" changes', () => {
      ...
      component.ngOnChanges(changesFake as any);
    ...
    it('setBadgeValue: should call checkBadgeValue', () => {
      const checkBadgeValueSpy = vi.spyOn(component as any, 'checkBadgeValue');
      component.value = 2;

      component['setBadgeValue']();

      expect(checkBadgeValueSpy).toHaveBeenCalledWith(component.value);
    });
