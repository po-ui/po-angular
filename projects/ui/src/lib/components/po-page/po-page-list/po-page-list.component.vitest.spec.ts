...
describe('PoPageListComponent - Desktop:', () => {
  ...
  it('should be call method of parent passsing function', () => {
    const element = desktopFixture.debugElement.nativeElement;
    const poButton = element.querySelectorAll('po-button > button')[0];

    const spy = vi.spyOn(poButton, 'dispatchEvent').mockImplementation(() => true);
  ...
  it('should call callActionFilter on key press', () => {
    const spy = vi.spyOn(component, 'callActionFilter').mockImplementation(() => {});
  ...
  it('should be change model', async () => {
    ...
    await desktopFixture.whenStable();
  ...
  it('onChangeDisclaimerGroup: should call recalculateHeaderSize when have disclaimers and isRecalculate is true', () => {
    ...
    const spy = vi.spyOn(component.poPageContent, 'recalculateHeaderSize').mockImplementation(() => {});
  ...
  it('onChangeDisclaimerGroup: should not call recalculateHeaderSize when not have disclaimers and call change of disclaimerGroup', () => {
    ...
    const recalculateSpy = vi.spyOn(component.poPageContent, 'recalculateHeaderSize').mockImplementation(() => {});
    const changeSpy = vi.spyOn(component.disclaimerGroup, 'change');
  ...
  it('onRemoveDisclaimer: should call disclaimerGroup.remove if it`s defined', () => {
    ...
    const spy = vi.spyOn(component.disclaimerGroup, 'remove');

    component.onRemoveDisclaimer(removeData);

    expect(spy).toHaveBeenCalledWith(removeData);
  });
  ...
  it('onRemoveAllDisclaimers: should call disclaimerGroup.removeAll if it`s defined', () => {
    ...
    const spy = vi.spyOn(component.disclaimerGroup, 'removeAll');

    component.onRemoveAllDisclaimers(removedDisclaimers);

    expect(spy).toHaveBeenCalledWith(removedDisclaimers);
  });
  ...
  describe('Templates:', () => {
    ...
    it('should show only one icon in button actions.', () => {
      component.actions[0].icon = 'an-newspaper';
      component.actions[1].icon = 'an-newspaper';
      component.actions[2].icon = 'an-newspaper';

      fixture.detectChanges();

      const icons = nativeElement.querySelectorAll('.an-newspaper');
  ...
  describe('Methods:', () => {
    it('callActionFilter: should call `filter.action` and `ChangeDetectorRef.detectChanges`', () => {
      ...
      const filterActionSpy = vi.spyOn(component.filter, fieldProperty as any);

      component.callActionFilter(fieldProperty);

      // `toHaveBeenCalledBefore` não existe no Vitest: compara a ordem de invocação dos spies.
      expect(filterActionSpy.mock.invocationCallOrder[0]).toBeLessThan(changeDetectorSpy.mock.invocationCallOrder[0]);
    ...
    it('callAction: should open an external URL in a new tab in the browser by calling Utils`s openExternalLink method', () => {
      ...
      const spy = vi.spyOn(UtilsFunction, 'openExternalLink').mockImplementation(() => {});
    ...
    it('filterSizeClass: should return `po-sm-6 po-md-6 po-lg-6 po-xl-6` when filter is empty', () => {
      component.filter = {};
      expect(component.filterSizeClass(6)).toBe('po-sm-6 po-md-6 po-lg-6 po-xl-6');
    });
    ...
    it('hasCustomFilterSize: should return `false` when filter.width is below range', () => {
      component.filter = { width: 0 };
      expect(component.hasCustomFilterSize()).toBe(false);
    });

    it('hasCustomFilterSize: should return `false` when filter.width is above range', () => {
      component.filter = { width: 99 };
      expect(component.hasCustomFilterSize()).toBe(false);
    });

    it('hasCustomFilterSize: should return `false` when filter is empty', () => {
      component.filter = {};
      expect(component.hasCustomFilterSize()).toBe(false);
    });

    it('hasCustomFilterSize: should return `false` when doesn`t have filter', () => {
      component.filter = undefined;
      expect(component.hasCustomFilterSize()).toBe(false);
    });
