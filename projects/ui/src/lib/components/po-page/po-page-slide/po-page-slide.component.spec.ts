...
describe('PoPageSlideComponent', () => {
  ...
  describe('ARIA:', () => {
    ...
    it('should link the accessible name via `aria-labelledby` when a title is defined', () => {
      component.title = 'My title';
      component.open();
      fixture.detectChanges();

      const container = debugElement.query(By.css('.po-page-slide-container')).nativeElement;
      const titleSpan = debugElement.query(By.css('.po-page-slide-title span')).nativeElement;

      expect(container.getAttribute('aria-labelledby')).toBe(component.titleId);
      expect(titleSpan.getAttribute('id')).toBe(component.titleId);
      expect(container.getAttribute('aria-label')).toBeNull();
    });

    it('should fallback to the default `aria-label` literal when there is no title', () => {
      ...
      expect(container.getAttribute('aria-label')).toBe(component.defaultDialogLabel);
      expect(component.defaultDialogLabel).toBeTruthy();
      expect(container.getAttribute('aria-labelledby')).toBeNull();
    });

    it('titleId: should be unique per component instance', () => {
      const secondFixture = TestBed.createComponent(PoPageSlideComponent);
      const secondComponent = secondFixture.componentInstance;

      expect(component.titleId).not.toBe(secondComponent.titleId);
    });
  });
