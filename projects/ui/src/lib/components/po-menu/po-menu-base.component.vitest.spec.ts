...
describe('PoMenuBaseComponent:', () => {
  ...
  describe('Methods: ', () => {
    it('configService: should call `menuService.configProperties` and set `filterService` if service parameter is valid string', () => {
      ...
      const spyConfigPropeties = vi.spyOn(component.menuService as any, 'configProperties' as any);
    ...
    it('setMenuExtraProperties: should call `removeBadgeAlert` three times', () => {
      ...
      vi.spyOn(component as any, 'removeBadgeAlert' as any).mockImplementation(() => {});
    ...
    it('processSubItems: should call `setMenuBadgeAlert` one time with `menu` and `menuItem2`', () => {
      ...
      vi.spyOn(component as any, 'setMenuItemProperties' as any).mockImplementation(() => {});
      vi.spyOn(component as any, 'setMenuBadgeAlert' as any).mockReturnValue(menus);
    ...
    it('processSubItems: should return a new reference of menu subItem', () => {
      ...
      vi.spyOn(Object, 'assign').mockReturnValue(menuAssigned);
  ...
  describe('Properties:', () => {
    it('service: should set property with `undefined` if invalid values', () => {
      ...
      const spyCofingService = vi.spyOn(component as any, 'configService' as any).mockImplementation(() => {});
    ...
    it('menuid: should call sendId', () => {
      vi.spyOn(component['menuGlobalService'] as any, 'sendId' as any).mockImplementation(() => {});
    ...
    it('collapsed: should call `validateCollapseClass`', () => {
      vi.spyOn(component as any, 'validateCollapseClass' as any).mockImplementation(() => {});
    ...
    describe('componentsSize', () => {
      ...
      it('onThemeChange: should call applySizeBasedOnA11y', () => {
        vi.spyOn(component as any, 'applySizeBasedOnA11y' as any).mockImplementation(() => {});
        component['onThemeChange']();
        expect((component as any).applySizeBasedOnA11y).toHaveBeenCalled();
      });
    });

    it('menus: should update property with valid values.', async () => {
      const validValues = [[{ label: 'Level 1.1', link: '/level-1-1', icon: 'clock' }]];

      const spyCheckingRouterChildrenFragments = vi
        .spyOn(component as any, 'checkingRouterChildrenFragments' as any)
        .mockImplementation(() => {});
      const spyCheckActiveMenuByUrl = vi
        .spyOn(component as any, 'checkActiveMenuByUrl' as any)
        .mockImplementation(() => {});

      const spySendMenus = vi
        .spyOn(component.menuGlobalService as any, 'sendMenus' as any)
        .mockImplementation(() => {});
  ...
  describe('Integration:', () => {
    // Skipped: complex spy timing between menus setter → setMenuExtraProperties → setMenuItemProperties
    it.skip('setMenuExtraProperties: should create menu with `badgeAlert` and structure it correctly', () => {
      vi.spyOn(component as any, 'setMenuItemProperties' as any).mockImplementation(() => {});
      ...
      // menus setter already called setMenuExtraProperties, so just verify the result
