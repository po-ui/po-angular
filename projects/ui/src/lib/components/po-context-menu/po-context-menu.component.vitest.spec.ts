...
describe('PoContextMenuComponent:', () => {
  ...
  describe('Methods:', () => {
    ...
    describe('selectItem:', () => {
      ...
      it('should call the action of the selected item', () => {
        ...
        expect(actionSpy).toHaveBeenCalledWith({ ...items[0], selected: true });
      });
      ...
      it('should emit itemSelected event when an item is selected', () => {
        ...
        expect(emitSpy).toHaveBeenCalledWith({ ...items[0], selected: true });
      });
    });
    ...
    describe('applyTooltipIfOverflows', () => {
      it('should set the value in signal if there is overflow', () => {
        const mockElement = {
          scrollHeight: 100,
          offsetHeight: 50
        } as HTMLElement;

        const mockSignal = signal<string | null>(null) as WritableSignal<any>;
        const text = 'Texto longo';

        component['applyTooltipIfOverflows'](mockElement, text, mockSignal);

        expect(mockSignal()).toBe(text);
      });

      it('should not set the value in signal if there is no overflow', () => {
        const mockElement = {
          scrollHeight: 50,
          offsetHeight: 50
        } as HTMLElement;
        ...
        component['applyTooltipIfOverflows'](mockElement, 'Texto', mockSignal);

        expect(mockSignal()).toBeNull();
      });
    });
  ...
  describe('Templates:', () => {
    ...
    it('should call toggleExpand when toggle button is clicked', () => {
      ...
      const toggleSpy = vi.spyOn(component, 'toggleExpand');
    ...
    it('should show aria-label when expanded', () => {
      component.expanded.set(true);
      fixture.detectChanges();

      const toggleBtn = nativeElement.querySelector('.po-context-menu-toggle');
      expect(toggleBtn.getAttribute('aria-label')).toBe(component.literals.close);
    });

    it('should show aria-label when collapsed', () => {
      component.expanded.set(false);
      fixture.detectChanges();

      const toggleBtn = nativeElement.querySelector('.po-context-menu-toggle');
      expect(toggleBtn.getAttribute('aria-label')).toBe(component.literals.open);
    });
