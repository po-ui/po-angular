...
export class PoTreeViewItemContentComponent {
  ...
  @ViewChild('inputCheckbox') protected inputCheckbox;
  @ViewChild('lineElement', { static: false }) protected lineElement: ElementRef<HTMLElement>;
  ...
  protected isLabelTruncated = false;
  ...
  protected get isSelected(): boolean {
    if (!this.selectable()) {
      return !this.hasSubItems && this.item()?.value === this.selectedValue();
    }

    // Single select segue seleção única comparando com o selectedValue.
  ...
  protected onLineFocus(event: FocusEvent) {
    this.updateLabelTruncation(event.currentTarget as HTMLElement);
    this.keyboardService.setLastFocusedNode(event.currentTarget as HTMLElement);
  }

  protected onLineMouseEnter(event: MouseEvent) {
    this.updateLabelTruncation(event.currentTarget as HTMLElement);
  }

  private updateLabelTruncation(node: HTMLElement) {
    const labelElement = node?.querySelector<HTMLElement>(
      '.po-tree-view-item-content-label, .po-tree-view-item-content-selectable .po-label'
    );

    this.isLabelTruncated = !!labelElement && labelElement.scrollHeight > labelElement.clientHeight;
  }
  ...
  private applyPressedState(node: HTMLElement) {
    ...
    const contentElement = node.querySelector<HTMLElement>('.po-tree-view-item-content');

    contentElement.classList.add(pressedClass);

    setTimeout(() => contentElement.classList.remove(pressedClass), 150);
  }
