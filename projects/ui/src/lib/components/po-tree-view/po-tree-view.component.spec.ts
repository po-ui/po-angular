...
describe('PoTreeViewComponent:', () => {
  ...
  describe('announce (aria-live): ', () => {
    it('onExpand: should announce expanded literal when treeViewItem.expanded is true', () => {
      const treeViewItem: PoTreeViewItem = { label: 'Nivel 01', value: 1, expanded: true };
      spyOn(component['treeViewService'], 'onExpand').and.returnValue(of(treeViewItem));
      spyOn(component, <any>'emitExpanded');

      component.ngOnInit();

      expect(component['ariaLiveMessage']).toBe(`Nivel 01, ${component['literals'].expanded}`);
    });

    it('onExpand: should announce collapsed literal when treeViewItem.expanded is false', () => {
      const treeViewItem: PoTreeViewItem = { label: 'Nivel 01', value: 1, expanded: false };
      spyOn(component['treeViewService'], 'onExpand').and.returnValue(of(treeViewItem));
      spyOn(component, <any>'emitExpanded');

      component.ngOnInit();

      expect(component['ariaLiveMessage']).toBe(`Nivel 01, ${component['literals'].collapsed}`);
    });

    it('onSelect: should announce selected literal when treeViewItem.selected is true', () => {
      const treeViewItem: PoTreeViewItem = { label: 'Nivel 01', value: 1, selected: true };
      spyOn(component['treeViewService'], 'onSelect').and.returnValue(of(treeViewItem));
      spyOn(component, <any>'emitSelected');

      component.ngOnInit();

      expect(component['ariaLiveMessage']).toBe(`Nivel 01, ${component['literals'].selected}`);
    });

    it('onSelect: should not announce when treeViewItem.selected is false', () => {
      const treeViewItem: PoTreeViewItem = { label: 'Nivel 01', value: 1, selected: false };
      spyOn(component['treeViewService'], 'onSelect').and.returnValue(of(treeViewItem));
      spyOn(component, <any>'emitSelected');

      component['ariaLiveMessage'] = '';
      component.ngOnInit();

      expect(component['ariaLiveMessage']).toBe('');
    });

    it('onActivate: should announce selected literal', () => {
      const treeViewItem: PoTreeViewItem = { label: 'Nivel 01', value: 1 };
      spyOn(component['treeViewService'], 'onActivate').and.returnValue(of(treeViewItem));
    ...
    it('announce: should update ariaLiveMessage and call markForCheck', () => {
      const spyMarkForCheck = spyOn(component['changeDetector'], 'markForCheck');

      component['announce']({ label: 'Item', value: 1 }, 'selecionado');

      expect(component['ariaLiveMessage']).toBe('Item, selecionado');
