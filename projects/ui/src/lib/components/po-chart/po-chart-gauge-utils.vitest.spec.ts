...
describe('PoChartGaugeUtils', () => {
  ...
  let componentMock: {
    getCSSVariable: ReturnType<typeof vi.fn>;
    el: ElementRef;
    series: any[];
    options: any;
    isGaugeSingle: boolean;
    itemsTypeGauge: any[];
    itemsColorTypeGauge: string[];
  ...
  beforeEach(() => {
    componentMock = {
      ...
      itemsTypeGauge: [],
      itemsColorTypeGauge: [],
      valueGaugeMultiple: 75,
      height: 500
    };

    gaugeUtils = new PoChartGaugeUtils(componentMock as unknown as PoChartComponent);
  });
  ...
  describe('setGaugeOptions', () => {
    ...
    beforeEach(() => {
      ...
      innerComponentMock = {
        getCSSVariable: vi.fn().mockReturnValue('mock-css-var'),
        el: { nativeElement: nativeElementMock },
        series: [],
        options: {},
        isGaugeSingle: false,
        itemsTypeGauge: [],
      ...
      Object.defineProperty(innerComponentMock, 'el', {
        get: () => ({ nativeElement: nativeElementMock })
      });

      utils = new PoChartGaugeUtils(innerComponentMock as unknown as PoChartComponent);
    });

    it('should return if isGaugeSingle is true', () => {
      innerComponentMock.isGaugeSingle = true;
      const options = {};
    ...
    it('should return if options.legend is false', () => {
      innerComponentMock.isGaugeSingle = false;
      innerComponentMock.options = { legend: false };
      const options = {};
    ...
    it('should return if series do not contain any item with label', () => {
      innerComponentMock.isGaugeSingle = false;
      innerComponentMock.options = { legend: true };
      innerComponentMock.series = [{ label: undefined }];
      const options = {};
    ...
    it('should set options.graphic with gauge legend items when valid conditions', () => {
      innerComponentMock.isGaugeSingle = false;
      innerComponentMock.options = {
        legend: true,
        showFromToLegend: true
      };

      innerComponentMock.series = [{ label: 'A' }];
      innerComponentMock.itemsTypeGauge = [{ label: 'A', from: 0, to: 10 }];
      innerComponentMock.itemsColorTypeGauge = ['#ff0000'];
    ...
    it('should wrap to next line when itemWidth exceeds chartWidth', () => {
      ...
      innerComponentMock.series = [{ label: 'A' }];
      innerComponentMock.itemsTypeGauge = [{ label: 'A', from: 0, to: 10 }];
      innerComponentMock.itemsColorTypeGauge = ['#00ff00'];
    ...
    it('should handle undefined, long, and short labels and wrap items correctly', () => {
      ...
      innerComponentMock.el.nativeElement.querySelector = () => chartElement;

      innerComponentMock.series = [{ label: 'Serie A' }];
      innerComponentMock.itemsTypeGauge = [{ label: 'Short' }, { label: 'A very very long label' }, { label: undefined }];
      innerComponentMock.itemsColorTypeGauge = ['#FF0000', '#00FF00', '#0000FF'];
  ...
  describe('setListTypeGauge', () => {
    let innerComponentMock: Partial<PoChartComponent>;
    let utils: PoChartGaugeUtils;
    let serie: any;
    ...
    beforeEach(() => {
      ...
      utils = new PoChartGaugeUtils(innerComponentMock as PoChartComponent);
      serie = {};
    });

    it('should configure serie correctly for single gauge', () => {
      innerComponentMock.series = [{ label: 'label1', data: 80 }];
      utils = new PoChartGaugeUtils(innerComponentMock as PoChartComponent);

      const result = utils.setListTypeGauge(serie, fontSizes);

      expect(innerComponentMock.isGaugeSingle).toBe(true);
      expect(innerComponentMock.itemsTypeGauge.length).toBe(1);
      expect(innerComponentMock.itemsTypeGauge[0].label).toBe('label1');
      expect(innerComponentMock.itemsTypeGauge[0].from).toBe(0);
      expect(innerComponentMock.itemsTypeGauge[0].to).toBe(80);
    ...
    it('should configure serie correctly for multiple gauges', () => {
      ...
      expect(innerComponentMock.isGaugeSingle).toBe(false);
      expect(utils['normalizeToRelativePercentage']).toHaveBeenCalledWith(innerComponentMock.series);
      expect(innerComponentMock.itemsTypeGauge.length).toBeGreaterThan(0);
      expect(result.type).toBe('gauge');
    ...
    it('should use fontSizeMd for small width or height', () => {
      innerComponentMock.series = [{ label: 'label1', data: 100 }];
      innerComponentMock.el.nativeElement.querySelector = vi.fn().mockReturnValue({ clientWidth: 400 });
      innerComponentMock.height = 300;
      utils = new PoChartGaugeUtils(innerComponentMock as PoChartComponent);
    ...
    it('should use fontSizeLg for large width and height', () => {
      innerComponentMock.series = [{ label: 'label1', data: 100 }];
      innerComponentMock.el.nativeElement.querySelector = vi.fn().mockReturnValue({ clientWidth: 500 });
      innerComponentMock.height = 600;
      utils = new PoChartGaugeUtils(innerComponentMock as PoChartComponent);
    ...
    it('should set divWidth to 0 when querySelector returns null', () => {
      innerComponentMock.series = [{ label: 'label1', data: 80 }];
      innerComponentMock.el.nativeElement.querySelector = vi.fn().mockReturnValue(null);
      utils = new PoChartGaugeUtils(innerComponentMock as PoChartComponent);
    ...
    it('should formatter return empty string when not single gauge and valueGaugeMultiple is falsy', () => {
      ...
      innerComponentMock.valueGaugeMultiple = 0;
      utils = new PoChartGaugeUtils(innerComponentMock as PoChartComponent);
    ...
    it('should use data.label as name if descriptionChart is undefined for single gauge', () => {
      innerComponentMock.series = [{ label: 'label1', data: 80 }];
      innerComponentMock.options.subtitleGauge = undefined;
      utils = new PoChartGaugeUtils(innerComponentMock as PoChartComponent);
  ...
  describe('finalizeSerieTypeGauge', () => {
    ...
    it('should handle multiple gauges using buildGaugeAxisLineColorsWithGaps', () => {
      ...
      vi.spyOn<any>(gaugeUtils, 'buildGaugeAxisLineColorsWithGaps').mockReturnValue([
        [0.25, '#ff0000'],
        [0.5, '#00ff00'],
        [1, '#0000ff']
      ]);
  ...
  describe('buildGaugeAxisLineColorsWithGaps', () => {
    ...
    it('should add final gap segment at the end when lastTo < maxTo', () => {
      ...
      vi.spyOn(Math, 'max').mockReturnValue(100);
