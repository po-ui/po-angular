...
describe('PoChartGaugeUtils', () => {
  ...
  describe('setListTypeGauge', () => {
      ...
      componentMock = {
        ...
        getCSSVariable: jasmine.createSpy('getCSSVariable').and.callFake(name => {
          const map = {
            ...
            '--font-weight-subtitle-gauge': '600',
            '--font-family-subtitle-gauge': 'Verdana',
            '--color-subtitle-gauge': '#666'
          };
          return map[name];
        })
      };
    ...
    it('should configure serie correctly for single gauge', () => {
      ...
      expect(result.data[0].name).toBe('descGauge');
    });
    ...
    it('should use data.label as name if descriptionGauge is undefined for single gauge', () => {
      componentMock.series = [{ label: 'label1', data: 80 }];
      componentMock.options.descriptionGauge = undefined;
      utils = new PoChartGaugeUtils(componentMock as PoChartComponent);
