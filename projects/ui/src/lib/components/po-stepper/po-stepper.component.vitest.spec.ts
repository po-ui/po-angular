...
describe('PoStepperComponent:', () => {
  ...
  describe('Methods:', () => {
    it('ngAfterContentInit: should call `activeFirstStep`', () => {
      const spyOnActiveFirstStep = vi.spyOn(component as any, 'activeFirstStep').mockImplementation(() => {});
    ...
    it('ngAfterContentInit: should call `controlStepsStatus` when `poSteps` is changed', () => {
      const spyOncontrolStepsStatus = vi.spyOn(component as any, 'controlStepsStatus').mockImplementation(() => {});

      vi.spyOn(component.poSteps.changes, 'subscribe').mockImplementation((callback: any) => {
        callback();
    ...
    it('active: shouldn`t call `getPoSteps` and `changeStep` if `usePoSteps` is false', () => {
      vi.spyOn(component, 'usePoSteps', 'get').mockReturnValue(false);

      const spyOnGetPoSteps = vi.spyOn(component as any, 'getPoSteps').mockImplementation(() => {});
      const spyOnChangeStep = vi.spyOn(component, 'changeStep').mockImplementation(() => {});
    ...
    it('active: should call `getPoSteps` and `changeStep` with `step` active if `usePoSteps` is true and `step` not is disabled', () => {
      vi.spyOn(component, 'usePoSteps', 'get').mockReturnValue(true);
      const position = 2;
    ...
    it(`changeStep: should call 'controlStepsStatus' and 'onChangeStep.emit' with 'step' if usePoSteps and
      ...
      const spyOncontrolStepsStatus = vi.spyOn(component as any, 'controlStepsStatus').mockImplementation(() => {});
      const spyOnChangeStep = vi.spyOn(component.onChangeStep, 'emit').mockImplementation(() => {});
    ...
    it('changeStep: shouldn`t call `onChangeStep.emit` if `stepNumber` param is same than `step`', () => {
      ...
      vi.spyOn(component, 'currentStepIndex', 'get').mockReturnValue(1);
    ...
    it('changeStep: should call `onChangeStep.emit` if `stepIndex` param is diff than `step` and `allowNextStep` return true', async () => {
      const stepIndex = 3;
      const stepMock = { id: 'step-3' } as PoStepComponent;
    ...
    it('should call `onChangeStep.emit` with `stepIndex + 1` if `usePoSteps` is false and stepIndex is different from currentStepIndex', async () => {
      const stepIndex = 2;
      const mockStep = { id: 'step-2' } as any;

      vi.spyOn(component, 'usePoSteps', 'get').mockReturnValue(false);
      vi.spyOn(component, 'currentStepIndex', 'get').mockReturnValue(0);
    ...
    it('isDashedBorderTop: should return true when the step status is done and the previous step is not done or active', () => {
      ...
      vi.spyOn(component, 'getPreviousPoSteps').mockReturnValue(previousStep);
      vi.spyOn(component, 'getPreviousSteps').mockReturnValue(previousStep);
    ...
    it('isDashedBorderTop: should return true when the step status is error and the next step is done', () => {
      ...
      vi.spyOn(component, 'getNextPoSteps').mockReturnValue(nextStep);
    ...
    it('isDashedBorderTop: should return true when the step status is default and the previous step is not done', () => {
      ...
      vi.spyOn(component, 'getPreviousPoSteps').mockReturnValue(previousStep as unknown as PoStepComponent);
      vi.spyOn(component, 'getPreviousSteps').mockReturnValue(previousStep);
    ...
    it('activeFirstStep: should call `changeStep` with first step if `usePoSteps` is true and has no step activated', () => {
      ...
      vi.spyOn(component, 'changeStep').mockImplementation(() => {});
    ...
    it('allowNextStep: should return of(false) if `sequential` is true, `hasStepWithCanActiveNextStep` is true, and `hasDefaultBeforeDone` is true', () => {
      component.sequential = true;
      const nextStepIndex = 1;
    ...
    it('allowNextStep: should return of(true) if `sequential` is true, `hasStepWithCanActiveNextStep` is true, but `hasDefaultBeforeDone` is false', () => {
      component.sequential = true;
      const nextStepIndex = 1;
    ...
    it(`canActiveNextStep: should return true if 'currentActiveStep.canActiveNextStep' function
      return an observable with true value`, () => {
    ...
    it(`canActiveNextStep: should return throw error and set 'currentActiveStep.status'
      with 'PoStepperStatus.Error' if some problem in observable`, () => {
    ...
    it(`canActiveNextStep: should return true and update the status to 'Done' if 'isBefore' is true and 'isCurrentStep' is false`, () => {
      const currentActiveStep = <PoStepComponent>{
        status: PoStepperStatus.Default,
    ...
    it(`canActiveNextStep: should return true and update the status to 'Default' if 'isCanActiveNextStep' is false`, () => {
      const currentActiveStep = <PoStepComponent>{
        status: PoStepperStatus.Default,
        canActiveNextStep: currentStep => of(false)
      };
    ...
    it(`controlStepsStatus: shouldn't call 'isBeforeStep', 'setStepAsActive', 'setNextStepAsDefault' and
      ...
      const spyOnIsBeforeStep = vi.spyOn(component as any, 'isBeforeStep').mockImplementation(() => {});
      const spyOnSetStepAsActive = vi.spyOn(component as any, 'setStepAsActive').mockImplementation(() => {});
      const spyOnSetNextStepAsDefault = vi.spyOn(component as any, 'setNextStepAsDefault').mockImplementation(() => {});
      const spyOnDetectChanges = vi.spyOn(component['changeDetector'], 'detectChanges').mockImplementation(() => {});
    ...
    it(`controlStepsStatus: should call 'setFinalSteppersAsDisabled' if 'usePoSteps' is true, 
      ...
      const spyOnSetFinalSteppersAsDisabled = vi.spyOn(component as any, 'setFinalSteppersAsDisabled').mockImplementation(() => {});
  ...
  it('getCanActiveNextStepObservable: should return result of canActiveNextStep', () => {
  ...
  it('getCanActiveNextStepObservable: should call canActiveNextStep of currentActiveStep if step.status is not Done', () => {
    const currentActiveStep = {
  ...
  describe('Integration:', () => {
    ...
    it(`should initialize first step as active, second as default and others as disabled,
      and should set step clicked as 'active', the previous as 'done', the next as 'default'and the last as 'disabled'.`, async () => {
      const poStepsMock = new QueryList<PoStepComponent>();
      const poStepList = [
        { id: '1', label: 'Step 1', status: PoStepperStatus.Active },
    ...
    it(`should initialize step 4 as active, step 5 as default and others as disabled`, async () => {
      const poStepsMock = new QueryList<PoStepComponent>();
      const poStepList = [
        { id: '1', label: 'Step 1', status: PoStepperStatus.Done },
