...
describe('PoDisclaimerGroupBaseComponent:', () => {
  ...
  describe('Methods:', () => {
    ...
    it('checkChanges: shouldn`t call `emitChangeDisclaimers` if `diff` return `false`.', () => {
      ...
      vi.spyOn(fakeThis, 'emitChangeDisclaimers');
      vi.spyOn(fakeThis.changeDetector, 'detectChanges');
    ...
    it('emitChangeDisclaimers: should call `change.emit` and set `previousDisclaimers` with `disclaimers`', () => {
    ...
    it(`onKeyPress: should call 'removeAllItems' if enter is typed.`, () => {
      ...
      vi.spyOn(component, 'removeAllItems');
