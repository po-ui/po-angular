...
describe('PoListViewBaseComponent:', () => {
  ...
  describe('Methods:', () => {
    ...
    it('selectListItem: should toggle off the current item when selection mode is `single`', () => {
      fixture.componentRef.setInput('p-selection-mode', 'single');
      fixture.detectChanges();
    ...
    it('p-actions-layout: should have `default` as default value', () => {
      expect(component.actionsLayout()).toBe(PoListViewActionsLayout.Default);
    });

    it('p-actions-layout: should update to `animalia` when set with `animalia`', () => {
      fixture.componentRef.setInput('p-actions-layout', 'animalia');
      fixture.detectChanges();

      expect(component.actionsLayout()).toBe(PoListViewActionsLayout.Animalia);
    });

    it('p-actions-layout: should fallback to `default` when set with an invalid value', () => {
      fixture.componentRef.setInput('p-actions-layout', 'invalid');
      fixture.detectChanges();

      expect(component.actionsLayout()).toBe(PoListViewActionsLayout.Default);
    });

    it('isAnimaliaActions: should return `true` when actions layout is `animalia`', () => {
      fixture.componentRef.setInput('p-actions-layout', 'animalia');
      fixture.detectChanges();

      expect(component.isAnimaliaActions).toBe(true);
    });

    it('isAnimaliaActions: should return `false` when actions layout is `default`', () => {
      expect(component.isAnimaliaActions).toBe(false);
    });
