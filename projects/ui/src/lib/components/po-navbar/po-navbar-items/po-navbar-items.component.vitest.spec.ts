...
describe('PoNavbarItemsComponent:', () => {
  ...
  it('should be created', () => {
    expect(component instanceof PoNavbarItemsComponent).toBe(true);
  });

  describe('Methods:', () => {
    it('ngOnDestroy: should call `unsubscribe` of the `routeSubscription`', () => {
      const spy = vi.spyOn(component['routeSubscription'], 'unsubscribe');
    ...
    it('ngOnInit: should call `subscribeToRoute`', () => {
      const spy = vi.spyOn(component as any, 'subscribeToRoute');
    ...
    it('checkRouterChildrenFragments: should return url without params and `#`', () => {
      vi.spyOn(component['router'], 'url', 'get').mockReturnValue('test/label#fragment?param=1');
    ...
    it('checkRouterChildrenFragments: should return `/` if `url` is `undefined`', () => {
      vi.spyOn(component['router'], 'url', 'get').mockReturnValue(undefined as any);
    ...
    it('checkRouterChildrenFragments: should return same url', () => {
      vi.spyOn(component['router'], 'url', 'get').mockReturnValue('test/label');
    ...
    it(`subscribeToRoute: should call checkActiveItemByUrl with url router if router events return an instance of NavigationEnd`, () => {
      ...
      component.items = [{ label: 'test', link: '/test' }];

      vi.spyOn(component as any, 'checkRouterChildrenFragments').mockReturnValue('test');
      const checkSpy = vi.spyOn(component as any, 'checkActiveItemByUrl').mockImplementation(() => {});

      vi.spyOn(component['router'].events, 'subscribe').mockImplementation((callback: any) => {
        callback(navigation);
      ...
      expect(checkSpy).toHaveBeenCalledWith('test');
    });
    ...
    it(`subscribeToRoute: shouldn't call checkActiveItemByUrl with url router if router events not return an instance of
      ...
      vi.spyOn(component as any, 'checkRouterChildrenFragments').mockReturnValue('test');
      const checkSpy = vi.spyOn(component as any, 'checkActiveItemByUrl');
  ...
  describe('Templates:', () => {
    it('should set `po-navbar-item-selected` class in active item', () => {
      ...
      expect(selectedItem.textContent.trim()).toBe('test 3');
    });
