...
describe('PoTextareaComponent:', () => {
  ...
  describe('Methods:', () => {
    ...
    describe('lockTextareaDimensions:', () => {
      it('should set textarea inline width and height when loading is true', () => {
        const textarea = component.inputEl.nativeElement;

        const expectedWidth = `${textarea.offsetWidth}px`;
        const expectedHeight = `${textarea.offsetHeight}px`;

        component.loading = true;
        component['lockTextareaDimensions']();

        expect(textarea.style.width).toBe(expectedWidth);
        expect(textarea.style.height).toBe(expectedHeight);
      });

      it('should clear textarea inline width and height when loading is false', () => {
        const textarea = component.inputEl.nativeElement;
