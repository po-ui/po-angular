...
describe('po-decimal-format.util: ', () => {
  describe('Methods: ', () => {
    ...
    describe('formatIntegerPart (truncated path): ', () => {
      it('should insert group separators from right when format is truncated by reduced integerDigitCount', () => {
        const parsed = parseDecimalFormat('>>>,>>>,>>9.99');
        ...
        expect(result.isValid).toBe(true);
        // Group separator inserted from right using original format positions
        expect(result.viewValue).toBe('1.234,56');
      });

      it('should not insert separator when digits are fewer than fromRight distance', () => {
        const parsed = parseDecimalFormat('>>>,>>>,>>9.99');
        const truncatedParsed = { ...parsed, integerDigitCount: 2 };
        const result = applyDecimalFormat(12.34, truncatedParsed, ',', '.');

        expect(result.isValid).toBe(true);
        // 2 digits, fromRight=3 and fromRight=6 → both posInDigits negative → no separator
        expect(result.viewValue).toBe('12,34');
      });

      it('should insert multiple separators correctly in truncated format', () => {
        const parsed = parseDecimalFormat('>>>,>>>,>>9.99');
        const truncatedParsed = { ...parsed, integerDigitCount: 7 };
        const result = applyDecimalFormat(1234567.89, truncatedParsed, ',', '.');

        expect(result.isValid).toBe(true);
        // 7 digits: pos6 → fromRight=3, posInDigits=4; pos3 → fromRight=6, posInDigits=1
        expect(result.viewValue).toBe('1.234.567,89');
      });
    });

    describe('insertGroupSeparatorsFromRight: ', () => {
      it('should return digits unchanged when groupSeparatorPositions is empty', () => {
        const parsed = parseDecimalFormat('>>>>>>>>9.99');
        // Format without commas → groupSeparatorPositions = []
        const result = insertGroupSeparatorsFromRight('1234567', parsed, '.');

        expect(result).toBe('1234567');
      });

      it('should return digits.join when input is array and no group separators', () => {
        const parsed = parseDecimalFormat('999');
        // No commas in format → groupSeparatorPositions = []
        const result = insertGroupSeparatorsFromRight(['0', '0', '7'], parsed, '.');

        expect(result).toBe('007');
      });

      it('should insert suppression placeholder instead of separator when preceding content is invisible', () => {
        const parsed = parseDecimalFormat('>>>,>>9.99');
        // Value "5" with format of 5 integer digits: padded to "\u200B\u200B\u200B\u200B5"
        // Position 3 from format: fromRight = 5-3=2, posInDigits = 5-2=3
        // Before position 3: ['\u200B', '\u200B', '\u200B'] → no visible content → should use placeholder
        const SUPPRESSION = '\u200B';
        const digits = [SUPPRESSION, SUPPRESSION, SUPPRESSION, SUPPRESSION, '5'];
        const result = insertGroupSeparatorsFromRight(digits, parsed, '.');

        // Separator position has no visible content before it, so placeholder is used
