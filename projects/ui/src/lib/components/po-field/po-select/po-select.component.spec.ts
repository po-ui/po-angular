...
describe('PoSelectComponent:', () => {
  ...
  describe('controlValueWithLabel', () => {
    ...
    it('should return null when calling getValueWrite when controlValueWithLabel is true and the object structure does not contain the value property', () => {
      const option = { id: 1, name: 'Xpto' };

      component.controlValueWithLabel = true;

      expect(component['getValueWrite'](option)).toBeNull();
    });
  });
});
