...
describe('Function callFunction:', () => {
  ...
  it('should call function of context', () => {
    vi.spyOn(context as any, 'getName' as any).mockImplementation(() => {});
  ...
  it('should call function of context passing attribute name', () => {
    vi.spyOn(context as any, 'getAge' as any).mockImplementation(() => {});
...
describe('accessibility level: ', () => {
  describe('getDefaultSize', () => {
    let poThemeServiceMock: { getA11yLevel: ReturnType<typeof vi.fn>; getA11yDefaultSize: ReturnType<typeof vi.fn> };

    beforeEach(() => {
      poThemeServiceMock = {
        getA11yLevel: vi.fn(),
        getA11yDefaultSize: vi.fn()
      };
    });
    ...
    it('should return `small` when the default component size is `small` in accessibility level AA (Service)', () => {
      ...
      expect(UtilFunctions.getDefaultSize(poThemeServiceMock as any, PoFieldSize)).toBe(PoFieldSize.Small);
    });
    ...
    it('should return `medium` if accessibility AA and default size is not set (Service)', () => {
      ...
      expect(UtilFunctions.getDefaultSize(poThemeServiceMock as any, PoFieldSize)).toBe(PoFieldSize.Medium);
    });
  ...
  describe('validateSize', () => {
    ...
    it('should return valid values based on accessibility level (Service)', () => {
      ...
      expect(UtilFunctions.validateSize(PoFieldSize.Small, poThemeServiceMock as any, PoFieldSize)).toBe(
        PoFieldSize.Small
      );
      expect(UtilFunctions.validateSize(PoFieldSize.Medium, poThemeServiceMock as any, PoFieldSize)).toBe(
        PoFieldSize.Medium
      );
    });
    ...
    it('should return valid values based on accessibility level', () => {
      ...
      expect(UtilFunctions.validateSizeFn(PoFieldSize.Small, PoFieldSize)).toBe(PoFieldSize.Small);
    ...
    it('should return the default size when an invalid value is provided (Service)', () => {
      ...
      expect(UtilFunctions.validateSize('xxg', poThemeServiceMock as any, PoFieldSize)).toBe(PoFieldSize.Medium);
    });
...
describe('mapInputSizeToLoadingIcon', () => {
  ...
  it('should return sm for invalid values', () => {
    expect(mapInputSizeToLoadingIcon('large')).toBe('sm');
    expect(mapInputSizeToLoadingIcon(undefined)).toBe('sm');
  });
});
