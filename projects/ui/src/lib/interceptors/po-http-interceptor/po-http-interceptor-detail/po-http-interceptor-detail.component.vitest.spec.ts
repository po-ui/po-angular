...
describe('PoHttpInterceptorDetailComponent:', () => {
  ...
  beforeEach(() => {
    ...
    // Mock the modal ViewChild since PoModalComponent is not declared (CUSTOM_ELEMENTS_SCHEMA)
    component.modal = {
      open: () => {},
      close: () => {}
    } as any;
  ...
  describe('Methods', () => {
    it('close: should call modal.close and closed.emit', () => {
      vi.spyOn(component.modal, 'close').mockImplementation(() => {});
    ...
    it('open: should call modal.open', () => {
      vi.spyOn(component.modal, 'open').mockImplementation(() => {});
