...
describe('mergePoI18nConfigs', () => {
  it('should prioritize the last defined default.language in the array', () => {
    ...
    expect(result.default).toEqual({ language: 'en-US', context: 'general', cache: true });
  });
  ...
  it('should fallback to last default if no default.language is defined', () => {
    ...
    const result = mergePoI18nConfigs(input);
    console.log('result.default.language)', result);
    expect(result.default).toEqual({ language: '', context: 'another-context', cache: false });
  ...
  it('should handle null or undefined contexts gracefully', () => {
    ...
    expect(result.default?.language).toBe('en-US');
    expect(result.default?.context).toBe('dashboard');
