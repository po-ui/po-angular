...
describe('PoI18nService:', () => {
  describe('without Service:', () => {
    ...
    it('should call getLiteralsFromContextService when servicesContext[context] exists', () => {
      ...
      vi.spyOn(service, 'getLiteralsFromContextService' as any).mockImplementation(() => {
    ...
    it('should return empty object to unexist context', () => {
      vi.spyOn(service, 'getLanguage').mockReturnValue('pt');
    ...
    describe('Methods:', () => {
      it('getLanguage: should call `languageService.getLanguage`.', () => {
        const languageServiceSpy = vi.spyOn(service['languageService'], 'getLanguage');
      ...
      it('getLanguage: should return `languageService.getLanguage` value.', () => {
        vi.spyOn(service['languageService'], 'getLanguage').mockReturnValue('pt-BR');

        expect(service.getLanguage()).toBe('pt-BR');
      });

      it('getShortLanguage: should call `languageService.getLanguage`.', () => {
        const languageServiceSpy = vi.spyOn(service['languageService'], 'getShortLanguage');
      ...
      it('getShortLanguage: should return `languageService.getShortLanguage` value.', () => {
        vi.spyOn(service['languageService'], 'getShortLanguage').mockReturnValue('pt');
      ...
      it('setLanguage: should call `languageService.setLanguage` with value language param if value is a language.', () => {
        ...
        vi.spyOn(service['languageService'], 'setLanguage').mockImplementation(() => {});
      ...
      it(`setLanguage: shouldn't call 'reloadCurrentPage' if value not is a language.`, () => {
        ...
        vi.spyOn(utils, 'isLanguage').mockReturnValue(false);
        vi.spyOn(utils, 'reloadCurrentPage').mockImplementation(() => {});
      ...
      describe('setConfig:', () => {
        it(`should call 'languageService.setLanguageDefault' with 'config.default.language' if 'config.default' is defined.`, () => {
          ...
          const languageServiceSpy = vi
            .spyOn(service['languageService'], 'setLanguageDefault')
            .mockImplementation(() => {});
      ...
      it('getLiterals: should call `getLanguage` to set language if `options.language` is undefined', () => {
        const storageLanguage = 'en';
        const params = [];

        vi.spyOn(service, 'getLanguage').mockReturnValue(storageLanguage);
        vi.spyOn(service, 'getLiteralsFromContextConstant' as any).mockImplementation(
      ...
      it(`getLiterals: shouldn't call 'getLanguage' and set language with 'options.language'
        if 'options.language' is defined`, () => {
        const options = { language: 'en' };
        const params = [];

        vi.spyOn(service, 'getLanguage').mockImplementation(() => 'pt');
        vi.spyOn(service, 'getLiteralsFromContextConstant' as any).mockImplementation(
          (language, context, literals, observer) => {
            params.push(context, literals, observer);
            observer.next();
    ...
    describe('Methods: ', () => {
      ...
      describe('getLiteralsFromContextService', () => {
        it(`should call 'observer.next' with translations if translations keys length is greater than 0
          ...
          const spyGetLiteralsLocalStorageAndCache = vi
            .spyOn(service, 'getLiteralsLocalStorageAndCache' as any)
            .mockImplementation(() => {});
          const spyMergeObject = vi.spyOn(service, 'mergeObject' as any).mockReturnValue(translations);
          const spyObserverNext = vi.spyOn(observer, 'next');
        ...
        it(`shouldn't call 'observer.next' with translations if translations keys length is 0
          ...
          const spyGetLiteralsLocalStorageAndCache = vi
            .spyOn(service, 'getLiteralsLocalStorageAndCache' as any)
            .mockImplementation(() => {});
          const spyMergeObject = vi.spyOn(service, 'mergeObject' as any).mockReturnValue({});
          const spyObserverNext = vi.spyOn(observer, 'next');
        ...
        it('should assign languageAlternative to languageSearch when languageAlternative is provided', () => {
          ...
          vi.spyOn(service, 'getLiteralsLocalStorageAndCache' as keyof PoI18nService).mockImplementation(() => {});
      ...
      describe('getLiteralsLocalStorageAndCache', () => {
        it(`should call 'searchInLocalStorage' and call 'observer.next' with
          mergedObject if 'useCache' is true`, async () => {
          const observer = { next: val => {} };
          ...
          const spySearchInLocalStorage = vi
            .spyOn(service, 'searchInLocalStorage' as any)
            .mockReturnValue(storageTranslations);
          const spyHttpService = vi.spyOn(service, 'getHttpService' as any).mockReturnValue(of(undefined));
        ...
        it(`should call 'searchInLocalStorage' and not call 'observer.next' with
          mergedObject if 'useCache' is true and localstorage is empty`, async () => {
          const observer = { next: val => {} };
          ...
          const spyObserverNext = vi.spyOn(observer, 'next');
          const spySearchInLocalStorage = vi.spyOn(service, 'searchInLocalStorage' as any).mockReturnValue([]);
          const spyHttpService = vi.spyOn(service, 'getHttpService' as any).mockReturnValue(of(undefined));
        ...
        it(`should call 'getHttpService' and call 'observer.next' with
          mergedObject from returned translations with current translation`, async () => {
          const observer = { next: val => {} };
          ...
          const spyObserverNext = vi.spyOn(observer, 'next');
          const spyHttpService = vi.spyOn(service, 'getHttpService' as any).mockReturnValue(of(translationsService));
        ...
        it(`should search for literals in portuguese if not all literals was found`, async () => {
          const observer = { next: val => {} };
          ...
          const spyCompleteFaultLiterals = vi
            .spyOn(service, 'completeFaultLiterals' as any)
        ...
        it(`should search for literals in portuguese if not all literals was found and languageAlternative is undefined`, async () => {
          const observer = { next: val => {} };
          ...
          const spyHttpService = vi.spyOn(service, 'getHttpService' as any).mockReturnValue(of(translationsService));
          const spygetLiteralsFromContextService = vi
            .spyOn(service, 'getLiteralsFromContextService' as any)
            .mockImplementation(() => {});
          service['getLiteralsLocalStorageAndCache'](
            'pt',
            'general',
      ...
      it('updateLocalStorage: should store values in localStorage when useCache is true', () => {
        ...
        expect(setItemSpy).toHaveBeenCalledWith('en-general-label1', 'Label 1');
        expect(setItemSpy).toHaveBeenCalledWith('en-general-label2', 'Label 2');
      });
    });
  });
});
