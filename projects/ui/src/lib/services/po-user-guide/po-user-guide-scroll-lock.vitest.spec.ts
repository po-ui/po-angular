...
const findListener = (spy: ReturnType<typeof vi.spyOn>, eventName: string): ((event: any) => void) | undefined => {
  const call = (spy as unknown as { mock: { calls: any[][] } }).mock.calls.find(args => args[0] === eventName);
...
describe('PoUserGuideScrollLock:', () => {
  ...
  describe('unlock:', () => {
    ...
    it('should remove wheel, touchmove and keydown listeners from document', () => {
      ...
      const eventTypes = removeSpy.mock.calls.map(args => args[0]);
  ...
  describe('isBrowser:', () => {
    it('should return true in the jsdom test environment', () => {
