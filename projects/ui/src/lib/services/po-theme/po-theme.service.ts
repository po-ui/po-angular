...
export class PoThemeService {
  ...
  private enableSmallSizeForComponents: boolean;
  ...
  /**
   * Define o tamanho `small` (altura de 32px) como padrão para componentes que suportam a medida e não possuem
   * a propriedade `p-size` definida. Aplica-se globalmente apenas quando o nível de acessibilidade for `AA`.
   * Caso contrário, o tamanho padrão será `medium` (altura de 44px).
   *
   * @param {boolean} enable Ativa ou desativa o tamanho `small` globalmente.
   */
  setA11yDefaultSizeSmall(enable: boolean) {
    const a11yLevel = document.documentElement.getAttribute('data-a11y');

    if (a11yLevel !== 'AA' && a11yLevel !== 'AAA') {
      this.enableSmallSizeForComponents = false;
      return;
    }

    this.enableSmallSizeForComponents = a11yLevel === 'AA' && enable;
  }
  ...
  getA11yDefaultSize(): string {
    return this.enableSmallSizeForComponents ? 'small' : 'medium';
  }
